...
class TrafficEnvironment:
    ...
    def step(self, action):
        ...
        # Ajouter de nouvelles voitures sans dépasser 9
        new_cars = np.random.randint(0, 3, size=4)
        for i in range(4):
            max_to_add = 9 - self.state[i]  # Calculer combien de voitures peuvent être ajoutées
            self.state[i] += min(new_cars[i], max_to_add)  # Ajouter sans dépasser 9
